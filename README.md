...
A responsive FAQ accordion built with semantic HTML and modern CSS. It uses the native `<details>` and `<summary>` elements to deliver an accessible, keyboard-friendly experience without JavaScript.
...
This project focuses on maintainable CSS through cascade layers, semantic design tokens, and a mobile-first workflow. It also demonstrates how native `<details>` and `<summary>` elements can replace JavaScript for common UI patterns while preserving accessibility.
